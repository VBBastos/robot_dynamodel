...
[uxl,uyl,uzl,wxl,wyl,wzl,time] = speeds();
size = length(time);
rate = 10;
...
uy = resample(uyl,time,rate);
uz = resample(uzl,time,rate);
wx = resample(wxl,time,rate);
wy = resample(wyl,time,rate);
...
time = 0:1/rate:floor(length(ux)/rate);
size=length(time);

%Plot Speeds
figure (2),
subplot(3,2,1), plot(time,ux(1:size));
title("ux");
subplot(3,2,3), plot(time,uy(1:size));
title("uy");
subplot(3,2,5), plot(time,uz(1:size));
title("uz");
subplot(3,2,2), plot(time,wx(1:size));
title("wx");
subplot(3,2,4), plot(time,wy(1:size));
title("wy");
subplot(3,2,6), plot(time,wz(1:size));
title("wz");
...
%%%%CTHETA%%%%
%%%%theta=[0.2604 0.2509 -0.000499 0.9965 0.00263 1.0768];
%ctheta = [0.242907508826870 0.358481953677894 0 0.995241614366413 0 0.984201811430821]
%ctheta = [0.2664    0.2688         0   1.0151    0.1098   1.0694];
...
plot(time,us,'g--');
hold off
lgd3 = legend("Reference Speed", "Output Speed", "Location", "Best")
lgd3.FontSize = LSIZE;
...
plot(time,ws,'g--');
hold off
legend("Estimated from SINDy", "Dataset", "Location", "Best")
xlabel('seconds');
ylabel('rad/s');
save('data','x','y','ures','wres','urefres','wrefres');
